...
@section('content')
    <div class="performancePage">
        <div class="pageS1">
            ...
            @component('new-theme.settings.performance-s.components.tabs')
                @slot('active', 'Period')

                <div class="tab-pane fade show active" id="pills-performancePeriod" role="tabpanel"
                    aria-labelledby="pills-performancePeriod-tab">
                    <div class='sectionS2'>
                        <div class="head withBorder flex align between">
                            <h3 class='small'>@lang('Performance Period')</h3>
                            <div class="flex align gap-3">

                                @can('PerformancePeriod-Create')
                                    <button class='buttonS1 primary' type="button" data-bs-toggle="modal" data-bs-target="#addNew">
                        ...
                        <div class="search">
                            <div class="inputS1 searchInput">
                                <input type="text" placeholder="Search...." value="{{ request('search') }}" id="search">
                        ...
                        <div class="tableS1 scroll">
                            <table>
                                <thead>
                                    <tr>
                                        <th>@lang('Performance Name')</th>
                                        <th>@lang('Performance Type')</th>
                                        <th>@lang('Action')</th>
                                    </tr>
                                </thead>
                                <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <div class='action flex gap-3'>
                                                    @can('PerformancePeriod-Edit')
                                                        <div data-bs-toggle="modal"
                                                            data-bs-target="#editperformanceperiod-{{ $performanceperiod->id }}">
                                                            <img src="{{ asset('new-theme/icons/all/edit.svg') }}" alt="" />
                                                        </div>
                                                    @endcan

                                                    @can('PerformancePeriod-Delete')
                                                        <div data-bs-toggle="modal"
                                                            data-bs-target="#delete-{{ $performanceperiod->id }}">
                    ...
                    <div class="paginater paginationS1 flex gap-4 align">
                        @include('new-theme.settings.performance-s.components.paginateperiod')
                    </div>
            ...
            @include('new-theme.settings.performance-s.components.addperiod')
        </div>
...
@push('script')
    <script>
        $(document).ready(function() {
            function fetch_data() {
                $.ajax({
                    url: "{{ route('PerformancePeriod.index') }}",
                    data: {
                        "search": $('#search').val()
                    },
