...
@section('content')
    <div class="branchesPage">
        <div class="pageS1">
            @component('new-theme.settings.salary.components.tabs')
                @slot('active', 'awardtype')
                <div class="tab-pane fade show active">
                    <div class='sectionS2'>
                        <div class="head withBorder flex align between">
                            <h3 class="small">{{__('Manage Award Type')}}</h3>

                            @can('AwardType-Create')
                                <button class='buttonS1 primary' data-bs-toggle="modal" data-bs-target="#addNewAwardType">
                        ...
                        <div class="tableS1 scroll">
                            <table>
                                <thead>
                                <tr>
                                    <th>@lang("Award Number")</th>
                                    <th>@lang("Award Type")</th>
                                    <th>@lang("Action")</th>
                                </tr>
                                </thead>
                                <tbody>
                                    @include("new-theme.settings.salary.paginations.awardtype_pagination")
                                </tbody>
                            </table>
                        </div>
                    </div>
    ...
    <!-- Add New award Modal -->
    <div class="modal fade customeModal" id="addNewAwardType" tabindex="-1" aria-labelledby="addNewAwardTypeLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-body">
                    <form class="formS1" method="post" action="{{ route('awardtype.store') }}">
                        @csrf
                        <div class="sectionS2">
                            <div class="head withBorder flex align between">
                                <h3 class='small'>@lang('Add New award type')</h3>
                                <button type="button" class="btn-close" data-bs-dismiss="modal"
                                    aria-label="Close"></button>
                            </div>
                            <div class="content">
                                <div class="">
                                    <label for="awardType" class="form-label">@lang("Award Type Arabic")</label>
                                    <div class="inputS1">
                                        <input name="name_ar" value="{{ old("name_ar") }}" type="text" id="awardType" placeholder='@lang("Enter Award Type Arabic")'>
                                    </div>
                                </div>

                                <div class="">
                                    <label for="awardTypeEn" class="form-label">@lang("Award Type English")</label>
                                    <div class="inputS1">
                                        <input name="name" value="{{ old("name") }}" type="text" id="awardTypeEn" placeholder='@lang("Enter Award Type English")'>
                                    </div>
                                </div>
