...
@section('content')
    <div class="branchesPage">
        <div class="pageS1">
            @component('new-theme.settings.salary.components.tabs')
                @slot('active', 'allowance')
                <div class="tab-pane fade show active">
                    <div class='sectionS2'>
                        <div class="head withBorder flex align between">
                            <h3 class="small">{{__("Manage Allowance Options")}}</h3>
                            @can('AllowanceOption-Create')
                                <button class='buttonS1 primary' data-bs-toggle="modal" data-bs-target="#addNewAllawance">
                        ...
                        <div class="tableS1 scroll">
                            <table>
                                <thead>
                                    <tr>
                                        <th>@lang('Allowance Number')</th>
                                        <th>@lang('Allowance Option')</th>
                                        <th>@lang('Action')</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @include("new-theme.settings.salary.paginations.allowanceoption_pagination")
                                </tbody>
                            </table>
                        </div>
                    </div>
    ...
    <div class="modal fade customeModal" id="addNewAllawance" tabindex="-1" aria-labelledby="addNewAllawanceLabel"
        aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-body">
                    <form class="formS1" method="post" action="{{ route('allowanceoption.store') }}">
                        @csrf
                        <div class="sectionS2">
                            <div class="head withBorder flex align between">
                                <h3 class='small'>@lang('Add New Allowance Option')</h3>
                                <button type="button" class="btn-close" data-bs-dismiss="modal"
                                    aria-label="Close"></button>
                            </div>
                            <div class="content">
                                <div class="">
                                    <label for="Allowanceoption" class="form-label">@lang('Allowance Option Arabic')</label>
                                    <div class="inputS1">
                                        <input name="name_ar" value="{{ old('name_ar') }}" type="text"
                                            id="Allowanceoption" placeholder='@lang('Enter Allowance Option Arabic')'>
                                    </div>
                                </div>

                                <div class="">
                                    <label for="AllowanceoptionEn" class="form-label">@lang('Allowance Option English')</label>
                                    <div class="inputS1">
                                        <input name="name" value="{{ old('name') }}" type="text"
                                            id="AllowanceoptionEn" placeholder='@lang('Enter Allowance Option English')'>
                                    </div>
                                </div>
...
@push('script')
    <script>
        $(document).ready(function() {
            function fetch_data() {
                $.ajax({
                    url: "{{ route('allowanceoption.index') }}",
                    data: {
                        "search": $('#search').val(),
                        "type": $('#type').val(),
                    },
