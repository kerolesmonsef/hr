...
@section('content')
    <div class="branchesPage">
        <div class="pageS1">
            @component("new-theme.settings.salary.components.tabs")
                @slot("active","deductionoption")
                <div class="tab-pane fade show active" >
                    <div class='sectionS2'>
                        <div class="head withBorder flex align between">
                            <h3 class="small">{{__('Manage Deduction Option')}}</h3>
                            
                            @can('DeductionOption-Create')
                                <button class='buttonS1 primary' data-bs-toggle="modal" data-bs-target="#addNewDeductionOption">
                        ...
                        <div class="tableS1 scroll">
                            <table>
                                <thead>
                                <tr>
                                    <th>@lang("Deduction Number")</th>
                                    <th>@lang("Deduction Option")</th>
                                    <th>@lang("Action")</th>
                                </tr>
                                </thead>
                                <tbody>
                                    @include("new-theme.settings.salary.paginations.deductionoption_pagination")
                                </tbody>
                            </table>
                        </div>
                    </div>
    ...
    <div class="modal fade customeModal" id="addNewDeductionOption" tabindex="-1" aria-labelledby="addNewDeductionOptionLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-body">
                    <form class="formS1" method="post" action="{{ route("deductionoption.store") }}">
                        @csrf
                        <div class="sectionS2">
                            <div class="head withBorder flex align between">
                                <h3 class='small'>@lang("Add New deduction option")</h3>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                            </div>
                            <div class="content">
                                <div class="">
                                    <label for="deductionOption" class="form-label">@lang("Deduction Option Arabic")</label>
                                    <div class="inputS1">
                                        <input name="name_ar" value="{{ old("name_ar") }}" type="text" id="deductionOption" placeholder='@lang("Enter Deduction Option Arabic")'>
                                    </div>
                                </div>

                                <div class="">
                                    <label for="deductionOptionEn" class="form-label">@lang("Deduction Option English")</label>
                                    <div class="inputS1">
                                        <input name="name" value="{{ old("name") }}" type="text" id="deductionOptionEn" placeholder='@lang("Enter Deduction Option English")'>
                                    </div>
                                </div>
...
@push('script')
    <script>
        $(document).ready(function() {
            function fetch_data() {
                $.ajax({
                    url: "{{ route('deductionoption.index') }}",
                    data: {
                        "search": $('#search').val(),
                    },
